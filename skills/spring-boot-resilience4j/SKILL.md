...
description: Provides fault tolerance patterns for Spring Boot 4.x using Resilience4j 2.2+. Use when implementing circuit breakers, handling service failures, adding retry logic with exponential backoff, configuring rate limiters, or protecting services from cascading failures. Generates circuit breaker, retry, rate limiter, bulkhead, time limiter, and fallback implementations. Validates resilience configurations through Actuator endpoints. Targets Boot 4 / Jakarta EE 11.
...
Provides Resilience4j 2.2+ patterns (circuit breaker, retry, rate limiter, bulkhead, time limiter, fallback) for Spring Boot 4.x fault tolerance with configuration and testing workflows.
