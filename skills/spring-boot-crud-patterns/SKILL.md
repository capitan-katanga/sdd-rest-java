...
description: Provides and generates complete CRUD workflows for Spring Boot 4.x services with Jakarta EE 11. Creates feature-focused architecture with Spring Data JPA (Hibernate 7.1) aggregates, repositories, DTOs, controllers, and REST APIs using jakarta.persistence + jakarta.validation annotations. Validates domain invariants and transaction boundaries. Use when modeling Java backend services, REST API endpoints, database operations, web service patterns, or JPA entities for Spring Boot applications. Targets Boot 4 / Jakarta EE 11.
...
Provides complete CRUD workflows for Spring Boot 4.x services using feature-focused architecture (Jakarta EE 11, Hibernate 7.1). Creates and validates domain aggregates, JPA repositories, application services, and REST controllers with proper separation of concerns. Defer detailed code listings to reference files for progressive disclosure.
