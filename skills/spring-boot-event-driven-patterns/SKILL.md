...
description: Provides Event-Driven Architecture (EDA) patterns for Spring Boot 4.x — creates domain events, configures ApplicationEvent and @TransactionalEventListener, sets up Kafka producers and consumers (spring-kafka 3.3+), and implements the transactional outbox pattern for reliable distributed messaging. Use when implementing event-driven systems in Spring Boot, setting up async messaging with Kafka, publishing domain events from DDD aggregates, or needing reliable event publishing with the outbox pattern. Targets Boot 4 / Spring Framework 7.
...
Implement Event-Driven Architecture (EDA) patterns in Spring Boot 4.x using domain events, ApplicationEventPublisher, `@TransactionalEventListener`, and distributed messaging with Kafka and Spring Cloud Stream.
