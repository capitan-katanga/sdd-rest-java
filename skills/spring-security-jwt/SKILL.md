...
description: "JWT/OAuth2 authentication and authorization for Spring Boot 4.x with Spring Security 7. Resource-server first (oauth2ResourceServer + JwtDecoder), Problem Details (RFC 7807) for auth errors, RBAC/ABAC, refresh-token rotation, CORS. Read before configuring SecurityFilterChain, BearerTokenAuthenticationFilter, @PreAuthorize, or any auth flow. Triggers: SecurityFilterChain, Spring Security 7, oauth2ResourceServer, JwtDecoder, JwtAuthenticationToken, BearerTokenAuthenticationFilter, JwtAuthenticationConverter, @PreAuthorize, @EnableMethodSecurity, @EnableWebSecurity, hasAuthority, hasRole, ProblemDetail, RFC 7807, jjwt, JJWT, refresh token rotation, CORS configuration, OAuth2 resource server."
allowed-tools: Read, Write, Edit, Bash, Glob, Grep
---

# Spring Security JWT (Boot 4 / Spring Security 7)

JWT/OAuth2 authentication and authorization patterns for Spring Boot 4.x using Spring Security 7. Covers resource-server configuration, token generation/validation, refresh strategies, RBAC/ABAC, OAuth2 integration, and Problem Details (RFC 7807) error responses.

> **Boot 4 source of truth**: see `references/spring-security-7-boot4.md` for the curated Spring Security 7 / Boot 4 cookbook (resource-server first, Problem Details, CORS, OAuth2). The other references describe a JJWT-based custom filter approach that remains valid for self-issued tokens.
...
- "Spring Security 7 configuration", "SecurityFilterChain setup", "oauth2ResourceServer"
...
### Spring Security 7 (Boot 4)
- Use `SecurityFilterChain` bean — `WebSecurityConfigurerAdapter` was removed in 6.x
- Prefer `.oauth2ResourceServer(rs -> rs.jwt(jwt -> jwt.decoder(jwtDecoder)))` over a custom `OncePerRequestFilter` when you can validate tokens with a `JwtDecoder` (issuer-based, JWKS, or symmetric)
- Disable CSRF only for stateless APIs; keep it enabled for session-based flows
- Use `@EnableMethodSecurity` (deprecated `@EnableGlobalMethodSecurity` is gone)
- Return Problem Details (RFC 7807) bodies on auth failures via a custom `AuthenticationEntryPoint` / `AccessDeniedHandler` — see `references/spring-security-7-boot4.md`
...
| [references/migration-spring-security-6x.md](references/migration-spring-security-6x.md) | Migration from Spring Security 5.x → 6.x (kept for projects still on Boot 3.x) |
| [references/spring-security-7-boot4.md](references/spring-security-7-boot4.md) | **Authoritative**: Spring Security 7 + Boot 4 cookbook (resource server, Problem Details, CORS, OAuth2) |
...
### Spring Security 7 Constraints (Boot 4)
- `WebSecurityConfigurerAdapter` is removed — use `SecurityFilterChain` beans only
- `@EnableGlobalMethodSecurity` is removed — use `@EnableMethodSecurity`
- Lambda DSL is required for `HttpSecurity` configuration (no method chaining)
- `@Order` on `@Configuration` classes drives filter chain ordering
- For multi-issuer setups use `JwtIssuerAuthenticationManagerResolver` rather than a custom resolver
