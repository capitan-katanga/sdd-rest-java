...
```
/sddrest.generate-refactoring-tasks $ARGUMENTS
```
...
- Primary: Use `sdd-rest-java:java-refactor-expert` agent with specialized knowledge of the task domain
- Or use appropriate specialized agent if available for the specific generation task

## Examples

```bash
/sddrest.generate-refactoring-tasks example-input
```